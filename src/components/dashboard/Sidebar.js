    ...
    await httpClient
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          localStorage.setItem('profileImage', res.data.user.profile_image);
        }
      })
      .catch((err) => {
        if (err.response) {
          ...
          <div className="profile_info hide_mob" id="tsparticles" style={{ position: 'relative' }}>
            <div className="profile_img">
              <div className="profile_img_div">
                ...
                <img src={userDetail ? (uploadedImage || localStorage.getItem('profileImage') || ProfileImage) : ProfileImage} alt="" />
              </div>
