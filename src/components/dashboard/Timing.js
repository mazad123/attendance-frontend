...
function Timing(props) {
	...
	const getGeoloaction = () => {
		...
		navigator.permissions.query({ name: "geolocation" }).then((result) => {
			if (result.state === "granted" || result.state === "prompt") {
				...
				setLocationEnabled(true); // Mark that permission is available
	...
	const getAddress = async (latitude, longitude) => {
		if (latitude && longitude) {
			...
			// );
			const response = await axios.get(
                `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&sensor=true&key=${GOOGLE_API_KEY}`
            );
